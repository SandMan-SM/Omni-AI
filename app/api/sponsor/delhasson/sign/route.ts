...
function isRecovery(v: unknown): v is "recover" | "waive" {
  return v === "recover" || v === "waive";
}
function isEquity(v: unknown): v is "discuss" | "later" {
  return v === "discuss" || v === "later";
}
...
export async function POST(req: Request) {
  ...
  if (!isRecovery(recovery)) {
    return NextResponse.json(
      { error: "Invalid recovery option" },
      { status: 400 },
    );
  }
  if (!isEquity(equity)) {
    return NextResponse.json(
      { error: "Invalid equity option" },
      { status: 400 },
    );
  }
  ...
  const recoveryLabel =
    recovery === "recover"
      ? "Yes — $200/mo recovery until 50% returned"
      : "Waive recovery (in favor of larger build / partnership track)";
  const equityLabel =
    equity === "discuss"
      ? "Discuss equity & partnership track"
      : "Not now — assets only";
  ...
  const html = `
    <div style="font-family: -apple-system, system-ui, Segoe UI, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #1f2937;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 0 0 20px; font-size: 14px;">
        ...
        <tr>
          <td style="padding: 10px 12px; background: #fef3c7; border: 1px solid #fde68a; font-weight: 700; color: #78350f;">Recovery</td>
          <td style="padding: 10px 12px; border: 1px solid #fde68a; color: #1f2937;">${escapeHtml(recoveryLabel)}</td>
        </tr>
        <tr>
          <td style="padding: 10px 12px; background: #fef3c7; border: 1px solid #fde68a; font-weight: 700; color: #78350f;">Equity track</td>
          <td style="padding: 10px 12px; border: 1px solid #fde68a; color: #1f2937;">${escapeHtml(equityLabel)}</td>
        </tr>
      </table>
  ...
  const delHtml = `
    <div style="font-family: -apple-system, system-ui, Segoe UI, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #1f2937;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 0 0 20px; font-size: 14px;">
        ...
        <tr><td style="padding: 10px 12px; background: #fef3c7; border: 1px solid #fde68a; font-weight: 700; color: #78350f;">Recovery</td><td style="padding: 10px 12px; border: 1px solid #fde68a; color: #1f2937;">${escapeHtml(recoveryLabel)}</td></tr>
        <tr><td style="padding: 10px 12px; background: #fef3c7; border: 1px solid #fde68a; font-weight: 700; color: #78350f;">Equity track</td><td style="padding: 10px 12px; border: 1px solid #fde68a; color: #1f2937;">${escapeHtml(equityLabel)}</td></tr>
