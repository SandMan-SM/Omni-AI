...
import { webinarRegistrationSchema, type WebinarRegistrationFormData } from "@/lib/schemas";
...
export function WebinarRegistrationModal({ isOpen, onClose }: WebinarRegistrationModalProps) {
  ...
  const form = useForm<WebinarRegistrationFormData>({
  ...
  const onSubmit = (data: WebinarRegistrationFormData) => {
    if (!selectedSession) {
