...
// SponsorTeaserClient — shared 7-second teaser surface used by
// every per-sponsor /sponsor/{slug} page (Del Hasson, Debbie
// Biery, Landon Hasson, Blake Hasson, Joshua Elkington,
// James Ball, Setefano Mafi, Supileo Mafi, Nele Mafi). Every
// hardcoded "Del Hasson" reference reads from props now, so
// each sponsor's page.tsx is a thin server-component wrapper
// that just plugs in name + brand hook + endpoints + date.
//
...
type Props = {
   ...
   *  Renders in the hero headline + the modal form label + the
   *  green success card. */
  sponsorName: string;
  /** Personalized brand hook for this sponsor — surfaced on the
   *  /info page §7 Why We Build paragraph, not on the teaser
   *  itself, but threaded through here for consistency with the
   *  info client's prop shape. */
  brandHook: string;
  /** Sign endpoint the modal POSTs to (e.g.
   *  `/api/sponsor/debbiebiery/sign`). Different per sponsor so
   *  the audit trail stays clean. */
  signEndpoint: string;
  /** Learn more CTA destination (e.g.
   *  `/sponsor/debbiebiery/info`). Different per sponsor. */
  infoHref: string;
  /** Sitani's pre-signed stamp date for this agreement (e.g.
   *  "May 22, 2026" for Del, "May 25, 2026" for the new
   *  batch). Static so SSR + client render the same string. */
          ...
          <div className="mx-auto max-w-5xl px-5 sm:px-8 py-6 sm:py-8">
            <div className="rounded-2xl border border-amber-300/30 bg-gradient-to-br from-amber-300/[0.05] via-amber-300/[0.02] to-transparent p-5 sm:p-6">
              ...
              <p className="mt-3 text-[15px] text-zinc-300 leading-relaxed">
                ...
                under <Link href={infoHref} className="text-amber-300 hover:text-amber-200 underline decoration-amber-300/40 underline-offset-4">Learn more</Link>.
              </p>
            </div>
          </div>
        </section>
