...
// LoveThyBarberReferralInfoClient — long-form companion to /lovethybarber/referral.
// Lives at /lovethybarber/referral/info (was /full pre-2026-05-24; URL
// renamed for parity with /renelaveau/referral/info). Carries the
// entire breakdown: leverage callout, chip strip, open-market
// value table, Love Thy Barber's live-build proof card, distribution +
// community grid, pricing reveal modal (two cards: $300 deposit +
// $3,000 full), why-this-is-different grid, AES-256 final CTA,
// and footer.
//
// The shorter /lovethybarber/referral page is the teaser that routes here
...
//   2. Proof — Love Thy Barber's live build card
...
function ping(action: string, target: string) {
  try {
    fetch(`${ANALYTICS_HOST}/api/inbound/ltb/events`, {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        properties: { asset: "ltb-referral", target },
      }),
    }).catch(() => {});
  } catch {
    /* fail open */
...
type Props = {
   ...
   * "Love Thy Barber" (the canonical Love Thy Barber featured creator). When the user
   * lands at /lovethybarber/referral/info?ref=jules (or =kimberly), the
...
export function LoveThyBarberReferralInfoClient({
  pageUrl,
  caseStudy,
  marketRates,
  marketTotal,
  ...
  function onPay(option: PricingOption) {
    ...
    // landed here via /lovethybarber/referral/info?ref=jules (or
    ...
    // ?ref=anything-unknown) defaults to "Love Thy Barber" and stays
    // unattributed, matching the bare /lovethybarber/referral surface.
    const stripeRef =
        ...
        <section className="relative">
          <div className="mx-auto max-w-7xl px-6 pt-20 pb-14 sm:pt-28 sm:pb-20">
            ...
            {/* Consolidated single intro paragraph — was two
                paragraphs (an "Love Thy Barber sent you" intro + a separate
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
              ...
              <Link
                ...
                data-testid="ltb-info-back-hero"
              >
                Back to Referral
              </Link>
            </div>
            ...
            {!pricingOpen && (
              <div className="mt-10 flex flex-col sm:flex-row gap-3 sm:gap-4 items-center justify-center">
                ...
                <Link
                  href="/lovethybarber/referral"
                  className="inline-flex items-center justify-center gap-3 rounded-2xl border border-amber-300/40 bg-amber-300/[0.04] hover:bg-amber-300/[0.10] px-6 sm:px-10 py-4 sm:py-5 text-sm font-semibold tracking-wide text-amber-200 transition-colors backdrop-blur-sm"
                  data-testid="ltb-info-back-inline"
                >
                  Back to Referral
                </Link>
              </div>
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-2 items-stretch max-w-3xl mx-auto text-left">
                {pricingOptions.map((opt) => (
                  <PricingCard
                    key={opt.id}
                    opt={opt}
                    surfaceTestId="ltb-pricing-inline"
                  />
                ))}
              </div>
            )}
            ...
            <div className="mt-10 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
                ...
                <p className="mt-2 text-xs text-zinc-400 leading-relaxed">
                  ...
                  ownership of the code Love Thy Barber&apos;s site runs on.
                </p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 items-stretch">
              {pricingOptions.map((opt) => (
                <PricingCard
                  key={opt.id}
                  opt={opt}
                  surfaceTestId="ltb-pricing-modal"
                />
              ))}
            </div>
