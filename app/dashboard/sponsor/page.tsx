...
const STAGES: Array<{ key: DealLead['deal_stage'] & string; label: string; color: string }> = [
  ...
  { key: 'closed_won',    label: 'Sponsor',       color: '#10b981' },
...
const PIPELINE_TYPE = 'sponsor';

export default function SponsorPipelinePage() {
  // Locked to Omni AI workspace — sponsor pipeline is Omni AI only.
    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', color: '#e8e8e8' }}>
      <header style={{ background: '#111', borderBottom: '1px solid #1e1e1e', padding: '0 32px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Handshake size={14} color="#fb923c" />
      ...
      <div style={{ padding: '24px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
          <Stat icon={DollarSign} label="Sponsor Pipeline (weighted)" value={fmtCurrency(totalPipeline)} sub="probability-adjusted" color="#a78bfa" />
          <Stat icon={Trophy} label="Sponsor Revenue" value={fmtCurrency(wonRevenue)} sub={`${byStage.closed_won.length} ${byStage.closed_won.length === 1 ? 'sponsor' : 'sponsors'}`} color="#10b981" />
